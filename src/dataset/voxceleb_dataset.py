...
from util.util import load_coeffs, load_landmarks, make_ids, read_target
...
class VoxCelebDataset(BaseDataset):
    def initialize(self, opt):
        ...
        self.landmark_img_generator = LandmarkImageGeneration(self.opt.output_size)

        self.total_person_id = len(self.person_ids)
        print(
            "\tnum videos: {}, repeat {} times, total: {}".format(
                self.total_person_id, opt.num_repeats, self.total_person_id * opt.num_repeats
            )
        )
