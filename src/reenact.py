...
from util.util import load_coeffs, load_landmarks, read_target, tensor2im
...
def parse_args():
    ...
    parser.add_argument(
        "--src_headpose",
        type=str,
        default="val_case/source_136/headpose.txt",
        help="input head pose coefficients of source image (.txt)",
    )
    parser.add_argument(
        "--src_landmark",
        type=str,
        default="val_case/source_136/landmark.txt",
        help="input facial landmarks of source image (.txt)",
    )
    parser.add_argument(
        "--drv_headpose",
        type=str,
        default="val_case/driving_521_512/headpose.txt",
        help="input head pose coefficients of driving image (.txt)",
    )
    parser.add_argument(
        "--drv_landmark",
        type=str,
        default="val_case/driving_521_512/landmark.txt",
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default=os.path.join("val_case", "result_136_521"),
...
if __name__ == "__main__":
    ...
    landmark_img_generator = LandmarkImageGeneration(opt.output_size)
