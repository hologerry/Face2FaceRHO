...
from models import VGG19_LOSS, networks
from models.base_model import BaseModel
from models.discriminator import MultiScaleDiscriminator
from models.image_pyramid import ImagePyramid
...
def define_discriminator(opt, init_type="normal", init_gain=0.02, gpu_ids=[]):
    discriminator = MultiScaleDiscriminator(
        scales=opt.disc_scales,
        num_channels=3,
        block_expansion=opt.disc_block_expansion,
        num_blocks=opt.disc_num_blocks,
        max_features=opt.disc_max_features,
        sn=False,
        use_kp=True,
    )
...
class Face2FaceRHOModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.pyramid = ImagePyramid(self.opt.loss_scales, 3).to(self.device)
            # initialize optimizers
            self.optimizers = []
            self.optimizer_rendering_net = torch.optim.Adam(
                self.rendering_net.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999)
            )
            self.optimizer_motion_net = torch.optim.Adam(
                self.motion_net.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999)
            )
            self.optimizer_discriminator = torch.optim.Adam(
                self.discriminator.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999)
            )
    ...
    def backward_G(self):
        ...
        if self.cur_epoch <= self.opt.init_field_epochs:
            init_field_loss_weight = 1000 * (self.opt.init_field_epochs - self.cur_epoch) / self.opt.init_field_epochs
    ...
    def calculate_loss(self, loss_criterion, input1, input2, pyramid_scales):
        ...
        loss = 0
        Pyramid_input1 = self.pyramid(input1)
        Pyramid_input2 = self.pyramid(input2)
        for scale in pyramid_scales:
            cur_loss = loss_criterion(
                Pyramid_input1["prediction_" + str(scale)],
                Pyramid_input2["prediction_" + str(scale)],
