...
class Trainer(object):
    ...
    def configure_optimizers(self):
        if self.train_detail:
            ...
            self.opt = torch.optim.Adam(self.deca.E_flame.parameters(), lr=self.cfg.train.lr, amsgrad=False)
    ...
    def training_step(self, batch, batch_nb, training_type="coarse"):
        ...
        if not self.train_detail:
            ...
            if self.cfg.loss.photo > 0.0:
                ...
                mask_face_eye = F.grid_sample(
                    self.deca.uv_face_eye_mask.expand(batch_size, -1, -1, -1),
                    opdict["grid"].detach(),
                    align_corners=False,
                )
                # images
        ...
        ###--------------- training detail model
            ...
            # mask
            mask_face_eye = F.grid_sample(
                self.deca.uv_face_eye_mask.expand(batch_size, -1, -1, -1), ops["grid"].detach(), align_corners=False
            )
            # images
            ...
            losses["photo_detail"] = (
                uv_texture_patch * uv_vis_mask_patch - uv_texture_gt_patch * uv_vis_mask_patch
            ).abs().mean() * self.cfg.loss.photo_D
            losses["photo_detail_mrf"] = (
                self.mrf_loss(uv_texture_patch * uv_vis_mask_patch, uv_texture_gt_patch * uv_vis_mask_patch)
            ...
            if self.cfg.loss.reg_sym > 0.0:
                ...
                losses["z_sym"] = (
                    nonvis_mask * (uv_z - torch.flip(uv_z, [-1]).detach()).abs()
    ...
    def validation_step(self):
        ...
        self.writer.add_image(
            "val_images", (grid_image / 255.0).astype(np.float32).transpose(2, 0, 1), self.global_step
        )
        self.deca.train()
    ...
    def evaluate(self):
        ...
        dataloader = DataLoader(dataset, batch_size=8, shuffle=False, num_workers=8, pin_memory=True, drop_last=False)
    ...
    def prepare_data(self):
        ...
        self.val_dataloader = DataLoader(
            self.val_dataset, batch_size=8, shuffle=True, num_workers=8, pin_memory=True, drop_last=False
        )
        self.val_iter = iter(self.val_dataloader)
            ...
            for step in tqdm(range(iters_every_epoch), desc=f"Epoch[{epoch+1}/{self.cfg.train.max_epochs}]"):
                ...
                if self.global_step % self.cfg.train.vis_steps == 0:
                    ...
                    visdict = {
                        ...
                        "landmarks2d_gt": util.tensor_vis_landmarks(
                            opdict["images"][visind], opdict["lmk"][visind], isScale=True
                        ),
                        "landmarks2d": util.tensor_vis_landmarks(
                            opdict["images"][visind], opdict["landmarks2d"][visind], isScale=True
                        ),
                    ...
                    # import ipdb; ipdb.set_trace()
                    self.writer.add_image(
                        "train_images", (grid_image / 255.0).astype(np.float32).transpose(2, 0, 1), self.global_step
                    )
                ...
                if self.global_step > 0 and self.global_step % self.cfg.train.checkpoint_steps == 0:
                    ...
                    if self.global_step % self.cfg.train.checkpoint_steps * 10 == 0:
                        ...
                        torch.save(
                            model_dict, os.path.join(self.cfg.output_dir, "models", f"{self.global_step:08}.tar")
                        )
